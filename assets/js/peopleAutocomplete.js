...
j("#instructor_auto").autocomplete("/course/peopleSearchAutocompleter/", {
...
j("#instructor_auto").result(function(event,data,formatted) {
  ...
  var display_name = parts[0] + ')';
	...
	var innerContents = entityType + "(" + entityId + ") <input type='button' value='X' onclick='clearAdminForm();'/>";
	...
	var innerContents = "<span class=\"new_user_span\" id='new_user_"+per_id+"'>" + entityType + 
											...
											"<input type='hidden' name='instructor_user[]' value='" + per_id + "' />" +
											"<br/></span>";
	
	 contents = j("#instructor_name_wrapper").html();
		j("#instructor_name_wrapper").html(contents + " " + innerContents);
		j("#instructor_auto").val("");
