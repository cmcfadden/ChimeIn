...
function updatePieChart() {
	...
	j.getJSON(window.location.href + "/true", function(data) {
		...
		for(var i in data.values) {
			var element_to_update = "#answer_" + data.values[i].label + "_count";

			if(j(element_to_update).text() != data.values[i].values[0]) {
				j(element_to_update).fadeOut('fast');
				j(element_to_update).text(data.values[i].values[0]);
				j(element_to_update).fadeIn('fast');
			}
		}
